...
3. **Ownership.** If `role` is already `OWNER`, skip to step 4. Otherwise the caller cannot revise or pull feedback (both are owner-gated):
...
5. Hand off: from here the plan behaves like one pushed from this session — use `/consensum-pull-feedback $PLAN_ID` for the human-in-the-loop cycle or `/consensum-loop $PLAN_ID` for autonomous operation.

Scope note: this hands over the *plan document*, not session state. Task files and implementation progress from the original author's machine do not travel.
